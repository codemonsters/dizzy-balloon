...
function rect_mt.__newindex(rect, key, value)
    if rect_mt["get_" .. key] then
        error(key .. " property/field alias is read-only. change fields instead")
    end
...
function rect_mt.get_width(self)
    return self.w
end
function rect_mt.get_height(self)
    return self.h
end
function rect_mt.get_left(self)
    return self.x
end
function rect_mt.get_top(self)
    return self.y
end
...
local binpacker_insert_prelude = (math.maxinteger and math.tointeger) and (function(self, w, h)
        ...
        if not w or not h then
            error("binpack:insert(w,h) had either w or h not coercable to integer (Lua 5.3+ path)")
        end
...
local binpacker_funcs = {
    ...
    _splitFreeNode = function(self, freeNode, usedNode)
        ...
        if not insideX or not insideY then
            return false
        end
...
local function binpacker_new(w, h)
    if not w or not h then
        error("must provide w,h to binpack new")
    end
