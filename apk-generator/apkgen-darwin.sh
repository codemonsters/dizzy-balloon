...
installapk() {
    ...
    appPath=$(./ptools-darwin/adb shell pm list packages es.codemonsters.dizzyballoon)
    if [ -z "$appPath" ]; then
        ./ptools-darwin/adb install db-android-aligned-debugSigned.apk;
    else 
        ./ptools-darwin/adb install -r db-android-aligned-debugSigned.apk;
    fi
}
...
devices=$(./ptools-darwin/adb devices | grep -w device || true)
if [ -z "$devices" ]; then
