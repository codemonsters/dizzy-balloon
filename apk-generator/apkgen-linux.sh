...
installapk() {
    ...
    appPath=$(./ptools-linux/adb shell pm list packages es.codemonsters.dizzyballoon)
    if [ -z "$appPath" ]; then
        ./ptools-linux/adb install db-android-aligned-debugSigned.apk;
    else 
        ./ptools-linux/adb install -r db-android-aligned-debugSigned.apk;
    fi
}
...
devices=$(./ptools-linux/adb devices | grep -w device || true)
if [ -z "$devices" ]; then
